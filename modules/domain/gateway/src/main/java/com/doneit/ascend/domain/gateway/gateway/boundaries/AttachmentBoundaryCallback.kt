...
class AttachmentBoundaryCallback(
    ...
    private val masterMindListModel: AttachmentsListModel
) : PagedList.BoundaryCallback<AttachmentEntity>() {
    ...
    override fun onItemAtEndLoaded(itemAtEnd: AttachmentEntity) {
        if (loadedCount < remoteCount) {
            fetchPage()
        }
    }

    override fun onItemAtFrontLoaded(itemAtFront: AttachmentEntity) {
    }

    private fun fetchPage() {
        scope.launch(Dispatchers.IO) {
            val response = remote.getAttachmentsList(masterMindListModel.toRequest(pageIndexToLoad))
