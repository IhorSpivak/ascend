...
class TagsAdapter(
    ...
    override fun getItem(position: Int): TagEntity? = types[position]
    ...
    override fun getView(position: Int, recycledView: View?, parent: ViewGroup): View {
        ...
        view?.let {
            ...
            it.title.text = types[position].tag
            if (position == 0) {
    ...
    override fun getDropDownView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        view?.let {
            if (it.tag != null) {
                (it.tag as FormatHolder).bind(types[position].tag)
            } else {
                val holder = FormatHolder(view)
                holder.bind(types[position].tag)
                it.tag = holder
            }
            return it
        }
